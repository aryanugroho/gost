...
func NewSQLStore(ctx context.Context, dbConfigMaster, dbConfigSlave db.Config) (Store, error) {
	...
	configSlave := db.Config{
		...
		MaxOpen:     int(dbConfigSlave.MaxOpen),
		MaxIdle:     int(dbConfigSlave.MaxIdle),
		MaxLifetime: int(dbConfigSlave.MaxLifetime),
		MaxIdleTime: int(dbConfigSlave.MaxIdleTime),
		// CA:          dbCa,
	...
	dbSlave, err := db.DB(configSlave, connectionTimeout)
	if err != nil {
		fmt.Println(err)
		return nil, err
	}
	...
	dbxSlave := sqlx.NewDb(dbSlave, "mysql")
